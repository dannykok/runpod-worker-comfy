...
class SupabaseJobTrigger(JobTrigger):
    ...
    service: Literal["supabase"] = Field(
        description="The service string is always 'supabase'")
    key_prefix: str = Field(
        description="The prefix of api key used in this job", min_length=1)
    ...
    output_field: str = Field(
        description="The output field name, usually to store the model output (e.g. url)")
    status_field: str | None = Field(description="The status field name")
    id: str = Field(
        description="The id value of the entry to be updated when the job completed successfuly")
    status: str | None = Field(
        description="The status value to be updated when the job completed successfuly")
...
class SupabaseTriggerHandler(TriggerHandler):
    ...
    def validate(self):
        ...
        if f"{self.trigger.key_prefix}SUPABASE_URL" not in os.environ:
            raise ValueError(
                f"{self.trigger.key_prefix}SUPABASE_URL not set in environment")
        if f"{self.trigger.key_prefix}SUPABASE_KEY" not in os.environ:
            raise ValueError(
                f"{self.trigger.key_prefix}SUPABASE_KEY not set in environment")
    ...
    def handle(self, output: str):
        ...
        data = {
            self.trigger.output_field: output
        }
        ...
        try:
            response = client.table(self.trigger.table).update(
                data).eq(self.trigger.id_field, self.trigger.id).execute()
